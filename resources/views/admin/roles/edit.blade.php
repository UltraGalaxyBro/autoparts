...
@section('content')
    ...
    <div class="mb-5">
        <form action="{{ route('roles.update', ['id' => $role->id]) }}" method="POST">
            ...
            <h4 class="fw-bold text-center mb-3">Editando função</h4>
            <div class="row justify-content-center mb-2">
                <div class="col-md-3">
                    <div class="form-floating mb-3">
                        <input type="text" class="form-control rounded-3" id="name" name="name"
                            placeholder="Sem abreviações" value="{{ old('name', $role->name) }}">
                        <label for="name">Nome</label>
                    </div>
                </div>
            </div>
            <div class="text-center">
                <button class="btn btn-lg btn-success rounded-3" title="Editar função" type="submit"
